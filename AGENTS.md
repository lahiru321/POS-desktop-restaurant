...
Guidance for Codex working in **`D:\Lumora\POS-desktop-restaurant`**.
...
> **The root `D:\Lumora\AGENTS.md` does not describe this directory.** It documents `POS System/`
...
POS-desktop-restaurant/
  ...
  lumora_pos/       Umbrella docs only (its AGENTS.md has the SaaS architecture/conventions)
```
...
- `lumora_pos/AGENTS.md` — SaaS architecture, multi-tenancy, auth, money-path invariants.
- `CLAUDE.md` — the same guidance for Claude Code; keep the two in step.
